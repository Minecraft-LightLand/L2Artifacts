...
public class ArtifactUpgradeManager {
	...
	public static void onUpgrade(ArtifactStats stats, int lv, Upgrade upgrade, RandomSource random) {
		...
		stats.add(stats.main_stat.type, stats.main_stat.getType().getMainValue(stats.rank, random, upgrade.removeMain()));
		if (lv % gate == 0 && stats.sub_stats.size() > 0) {
			...
			stats.add(substat.type, substat.getType().getSubValue(stats.rank, random, upgrade.removeSub()));
		}
	}
}
